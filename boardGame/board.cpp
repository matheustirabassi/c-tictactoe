...
int firstTurn(int **A) {
    if(checkWin(A) == 0) {
        if(randomNumber() < 50) {
            cout << "Primeiro jogador é a ";
            textcolor(RED);
            cout << "bolinha!" << endl << endl;
            textcolor(WHITE);
            return 1;
        }
        cout << "Primeiro jogador é o ";
        textcolor(BLUE);
...
int createGame() {
    ...
    printCorrectBoard(board);
    if(checkWin(board) == 1) {
        cout << "O jogador ganhador é a bolinha! (player 1)"<< endl << endl;
        freeGame(board);
        return 1;
    }

    if(checkWin(board) == 2) {
        cout << "O jogador ganhador é o xizinho! (player 2)"<< endl << endl;
